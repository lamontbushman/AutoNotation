...
public class idft {
  ...
  /**
    ...
    <p>
    The inverse DFT constructor is passed a vector of
    complex objects (note that this differs from the
    <tt>dft</tt> constructor which is passed a vector
    of points).
    </p>

   */
  public idft( Vector<Complex> d )
  {
    N = 0;
    data = null;
    if (d != null) {
...
/*	// check to see if its a Vector of "complex"
  ...
  /**
    ...
    <p> 
    The inverse DFT algorith is an N<sup>2</sup> algorithm.  For a
    DFT point at <i>m</i>, there are O(N) calculations.  The basic 
    inverse DFT equation for calculating the inverse DFT point 
    at <i>m</i> is shown below.  In this equation the complex
    data values that resulted from a DFT transform are stored in 
    the array "X".  X[n] is one element of this array.  
    "j" is the sqrt(-1) a.k.a the imaginary number i.  
    </p>
...
<p>
   At each step in the summation above the complex number
   X[n] is multiplied by the complex number
</p>
<pre>
