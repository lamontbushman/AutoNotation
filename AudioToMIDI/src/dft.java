...
public class dft {
  ...
  private Vector<point> data;  // Data on which the DFT is performed, 
...
<p>
    ...
    point objects.  There are a number of ways to regard the <i>x</i>
    and <i>y</i> values in the point object.  In the context of signal
    processing, the <i>x</i> values would be time, and the <i>y</i>
    values would be the magnitude of the signal at that time.  If a
    synthetic function, like <tt>sin(x)</tt> is sampled, the <i>x</i>
    component is the argument to the function and the <i>y</i>
    component is the result.
</p>
   */
  public dft( Vector<point> d )
  {
    N = 0;
    data = null;
    if (d != null) {
